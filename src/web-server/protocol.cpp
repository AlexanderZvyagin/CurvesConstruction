...
// auto logger = get_logger("protocol");
...
{
    // FuncTime ft("ws protocol callback function");
    logger->debug("callback function: START   len={}",len);
    // lws_print_reason(reason);
    ...
    try {
        switch (reason) {
            ...
            case LWS_CALLBACK_PROTOCOL_INIT: {
                ...
                // host_data->context = lws_get_context(wsi);
                // host_data->protocol = lws_get_protocol(wsi);
                // host_data->vhost = lws_get_vhost(wsi);
            ...
            case LWS_CALLBACK_ESTABLISHED: {
                ...
                logger->debug("new session: {}",reinterpret_cast<long int>(ud->session));
            ...
            case LWS_CALLBACK_SERVER_WRITEABLE: {
                ...
                // This code may be called several times for a fragmented message.
        ...
        lws_close_reason(
            ...
            strlen(e.what()) );
        logger->debug("callback function: END");
        return -1;
    }

    // logger->debug("callback function: END, time={} seconds",ft.RunSeconds());
    return 0;
}
